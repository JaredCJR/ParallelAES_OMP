...
void Kernel_AES_AddRoundKey(BYTE[], BYTE[]);
void Kernel_AES_SubBytes(BYTE[], BYTE[]);
void Kernel_AES_ShiftRows(BYTE[], BYTE[]);
...
#pragma omp declare target
void Kernel_AES_SubBytes(BYTE state[], BYTE sbox[])
{
	int i;
	for (i = 0; i < BLOCK_LENGTH; i++)
		state[i] = sbox[state[i]];
...
#pragma omp declare target
void Kernel_AES_AddRoundKey(BYTE state[], BYTE rkey[])
{
	int i;
	for (i = 0; i < BLOCK_LENGTH; i++)
		state[i] ^= rkey[i];
...
#pragma omp declare target
void Kernel_AES_ShiftRows(BYTE state[], BYTE shifttab[])
{
	BYTE h[BLOCK_LENGTH];
	memcpy(h, state, BLOCK_LENGTH);
	int i;
...
void AES_Encrypt(BYTE block[], BYTE key[], int keyLen)
{
    ...
    {    
    Kernel_AES_AddRoundKey(block, &key[0]);
...
//#pragma omp parallel for// schedule(static,1)
	...
	Kernel_AES_SubBytes(block, AES_Sbox);
	Kernel_AES_ShiftRows(block, AES_ShiftRowTab);
	Kernel_AES_AddRoundKey(block, &key[keyLen-BLOCK_LENGTH]);
    }
 }
#pragma omp end declare target
